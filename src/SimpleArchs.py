...
def get_simple_model(name,out_nodes,lin_layers = None,in_size=224):
    if(name=="LargeT"):
        return SimpleConv(out_nodes=out_nodes,final_max = False,lin_layers=lin_layers,in_size=in_size)
    elif(name=="LargeW"):
        return SimpleConv(out_nodes = out_nodes,min_layer_size = 64,lin_layers=lin_layers,in_size=in_size)
    elif(name=="Small"):
        return SimpleConv(out_nodes=out_nodes,min_layer_size=32,max_layer_size=256,lin_layers=lin_layers,in_size=in_size)
    elif(name=="Tiny"):
        return SimpleConv(out_nodes = out_nodes,min_layer_size=64,filter_size=5,lin_layers=lin_layers,in_size=in_size)
    elif(name=="Very_Tiny"):
        return SimpleConv(out_nodes=out_nodes,min_layer_size=32,max_layer_size=256,filter_size=9,avg_pool=False,in_size=in_size)
    else:
        return None
...
class SimpleConv(nn.Module):
    ...
    def __init__(self,out_nodes = 10,min_layer_size=32,max_layer_size=512,filter_size=7,final_max=True,max_pool_window=3,
    max_pool_stride=2,lin_layers = None,dropout_pct = 0.25,in_size = 224,
    avg_pool=True):
        super(SimpleConv,self).__init__()
        
        #Save layer filter sizes, layer strides, paddings for Proto Pnet
        self.layer_filter_sizes = []
        self.layer_strides = []
        self.layer_paddings = []
        ...
        #Convolutional layers, then other layers
        ...
        while(curr_layer < max_layer_size):
            ...
            self.layer_filter_sizes.append(filter_size)
            self.layer_strides.append(1)
            self.layer_paddings.append(0)
            ...
            if(curr_layer < max_layer_size or final_max):
                ...
                self.layer_filter_sizes.append(max_pool_window)
                self.layer_strides.append(max_pool_stride)
                self.layer_paddings.append(0)
            count += 1
        ...
        #Remember number of layers in final block
        self.num_linear = 0
        
        if(out_size[2]>1 and avg_pool):
